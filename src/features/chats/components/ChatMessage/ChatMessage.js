...
// import useSelfWriteText from "../../hooks/useSelfWriteText";
import SelfWriteText from "../SelfWriteText/SelfWriteText";

const ChatMessage = forwardRef(({message, selfWrite, setInputDisabled, setScrollIntoViewTrigger = () => null }, ref) => {
    return (
        <div 
        style={{ backgroundColor: message?.role === ChatMemberRole["GPT-3"] 
        ? "rgb(69, 70, 84)" : "transparent" }} ref={ref}>
            <div className="chat-message">
                <div className="author-logo">
                    {message?.role === ChatMemberRole["GPT-3"] ? 
                        <svg xmlns="http://www.w3.org/2000/svg"
                            ...
                            viewBox="0 0 512 512"><rect fill="#10A37F" width="512" height="512" 
                            rx="104.187" ry="105.042" /><path fill="#fff" fillRule="nonzero" 
                        ...
                        :
                        <PersonIcon style={{width:"2rem"}} />
                    }
                </div>
                {selfWrite ? 
                    <SelfWriteText text={message.text} setInputDisabled={setInputDisabled} setScrollIntoViewTrigger={setScrollIntoViewTrigger} /> :
                    <p className="message-text" style={{...message.role === ChatMemberRole["GPT-3"] && {color:"#d1d5db"}}}>
                        {message.text ?? ""}
                    </p>
                }
            </div>
